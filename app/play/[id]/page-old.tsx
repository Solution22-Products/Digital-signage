...
const DynamicSlider: React.FC<Props> = ({ params: { id } }) => {
  ...
  useEffect(() => {
    ...
    const playbackStateKey = "is_playing";

    // Retrieve or generate the device ID
    let device_id = sessionStorage.getItem(deviceIdKey);
    if (!device_id) {
      device_id = generateShortCode();
      sessionStorage.setItem(deviceIdKey, device_id); // Store the device_id in sessionStorage
    }

    // Retrieve the playback state from sessionStorage
    const storedPlaybackState = sessionStorage.getItem(playbackStateKey);
    ...
    const startPlayback = async () => {
      try {
        ...
        if (existingSession) {
          ...
          if (!updateError) {
            setIsPlaying(true);
            sessionStorage.setItem(playbackStateKey, "true");
...
*/
  ...
  if (isPlaying === false) {
    layoutContent = (
      <div className="h-screen w-screen">
        <div className="w-[100%] h-screen bg-black-500 flex items-center justify-center text-2xl font-bold">
          Screen is playing by some other device!
        </div>
      </div>
    );
  }
