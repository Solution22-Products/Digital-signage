...
const TurnOffScreen = () => {
  ...
  const validateForm = () => {
    ...
    // if (selectedScreenValues.length === 0)
  ...
  const handleCreateSchedule = async () => {
    ...
    const formatTime12HourOnly = (time: any) => {
    ...
    //   ? formatTime12HourOnly(startTime)
    //   : null;
    // const formattedEndTime = endTime ? formatTime12HourOnly(endTime) : null;
    ...
    if (error) {
      ...
      //notify("Turn off schedule created successfully", true);
      toast({
        title: "Created Successfully!.",
        description: "Turn off schedule created successfully!",
      });
      setTimeout(() => router.push("/schedule"), 3000);
    }
  };
  ...
  const handleChange = (e: any) => {
    ...
    // setErrors((prevErrors: any) => ({ ...prevErrors, repeatEvery: undefined }));
  };
    ...
    <>
      <div>
        <div>
          ...
          />
          {errors.scheduleName && (
            <p className="text-red-500 text-sm mt-1">{errors.scheduleName}</p>
          )}
        </div>
              ...
              <Popover open={screenOpen} onOpenChange={setScreenOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    {selectedScreenValues
                      ? screenList.find(
                          (screen: any) => screen.id === selectedScreenValues
                        )?.screenname
                      : "Select Screen"}
                            ...
                            {filteredScreens.map((screen: any) => (
                              <CommandItem
                                ...
                                onSelect={(currentValue) => {
                                  const selectedValue =
                                    currentValue === selectedScreenValues
                                      ? null
                                      : currentValue;
                                  setSelectedScreenValues(selectedValue);
